...
std::map<std::string, std::string> GlobalMsg
{
	...
	{"strGroupLicenseDeny","本群未获{self}许可使用，自动在群内静默。\n请先.help协议 阅读并同意协议后向运营方申请许可使用，\n否则请管理员使用!dismiss送出{self}\n可按以下格式填写并发送申请:\n!authorize 申请用途:[ **请写入理由** ] 我已了解OlivaDice(DIXE)基本用法，仔细阅读并保证遵守{strSelfName}的用户协议，如需停用指令使用[ **请写入指令** ]，用后使用[ **请写入指令** ]送出群" },
	...
	{"strPreserve", "{self}私有私用，勿扰勿怪\n如需申请许可请发送!authorize +[群号] 申请用途:[ **请写入理由** ] 我已了解OlivaDice(DIXE)基本用法，仔细阅读并保证遵守{strSelfName}的用户协议，如需停用指令使用[ **请写入指令** ]，用后使用[ **请写入指令** ]送出群"},
...
{"授权许可","&authoize"},
{"authorize","授权许可(非信任用户使用时转为向管理申请许可)\n!authorize (+[群号]) ([申请理由])\n群内原地发送可省略群号，无法自动授权时会连同理由发给管理\n默认格式为:!authorize 申请用途:[ **请写入理由** ] 我已了解OlivaDice(DIXE)基本用法，仔细阅读并保证遵守{strSelfName}的用户协议，如需停用指令使用[ **请写入指令** ]，用后使用[ **请写入指令** ]送出群"},
{"开关","&bot"},
...
.pc clr //销毁全部角色卡记录
	...
	{
		"房规",
		"OlivaDice(DIXE)目前只为COC7检定提供房规，指令为.setcoc:{setcoc}"
	},
	{
		"setcoc",
...
# 词库批量加载方式见手册)" },
	{"投喂","&投食"},
	{"投食", "投食仑质，请选择http://benzenpenxil.xyz/OlivaPay/"},
