...
int readJMap_old(const nlohmann::json& j, std::map<T1, T2, sort>& mapTmp)
{
	int intCnt = 0;
...
template <typename T>
int readJson_old(const std::string& strJson, std::set<T>& setTmp) {
	try {
...
int readJson_old(const std::string& strJson, std::map<T1, T2>& mapTmp)
{
	try
	{
		nlohmann::json j = nlohmann::json::parse(strJson);
		return readJMap_old(j, mapTmp);
	}
	catch (...)
	{
		return -1;
...
int loadJMap_old(const std::string& strLoc, std::map<T1, T2, sort>& mapTmp) {
...
template <typename T1, typename T2, typename sort>
int saveJMap_old(const std::string& strLoc, std::map<T1, T2, sort> mapTmp)
{
